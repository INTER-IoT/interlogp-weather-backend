...
const generate = (stationId, size, date) => fakeData.generateWeatherMeasurements(size, `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`, `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`)
  .map((measurement) => {
